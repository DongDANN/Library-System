...
Public Class AccountsManagement
    ...
    Private Sub HideSensitiveColumns()
        ...
        AccountManagementDataGrid.Columns("SecretQuestion").Visible = False
        AccountManagementDataGrid.Columns("SecretQuestionAnswer").Visible = False

    End Sub

    Private Function IsUsernameExists(username As String) As Boolean
        Dim query As String = "SELECT COUNT(*) FROM dbo.tblaccounts WHERE Username = @Username"
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        If txtPassword.Text = txtConfirmPassword.Text Then
            Try
                ...
                Dim checkQuery As String = "SELECT COUNT(*) FROM dbo.tblaccounts WHERE Name=@Name"
                cmd = New SqlCommand(checkQuery, con)
                cmd.Parameters.AddWithValue("@Name", txtName.Text)
                Dim count As Integer = CInt(cmd.ExecuteScalar())

                If count > 0 Then
                    Dim query As String = "UPDATE dbo.tblaccounts SET Username=@Username, Password=@Password, AccountType=@AccountType, Status=@Status WHERE Name=@Name"
    ...
    Private Sub btnNew_Click(sender As Object, e As EventArgs) Handles btnNew.Click
        If txtPassword.Text = txtConfirmPassword.Text Then
            If Not IsUsernameExists(txtUsername.Text) Then
                Try
                    con.Open()
                    Dim query As String = "INSERT INTO dbo.tblaccounts (Name, Username, Password, AccountType, Status) VALUES (@Name, @Username, @Password, @AccountType, @Status)"
                    cmd = New SqlCommand(query, con)
                    cmd.Parameters.AddWithValue("@Name", txtName.Text)
                    cmd.Parameters.AddWithValue("@Username", txtUsername.Text)
                    cmd.Parameters.AddWithValue("@Password", txtPassword.Text)
        ...
        If txtName.Text <> "" Then
            ...
            If result = DialogResult.Yes Then
                Try
                    ...
                    Dim archiveQuery As String = "INSERT INTO dbo.tblaccounts_archive (Name, Username, Password, AccountType, Status) " &
                                         "SELECT Name, Username, Password, AccountType, Status FROM dbo.tblaccounts WHERE Name=@Name"
                    ...
                    Dim deleteQuery As String = "DELETE FROM dbo.tblaccounts WHERE Name=@Name"
    ...
    Private Sub AccountsManagement_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.TblaccountsTableAdapter.Fill(Me.LibrarydbDataSet.tblaccounts)
        data()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs)
        con.Open()
        cmd = New SqlCommand("Select * from dbo.tblaccounts where Name='" + txtName.Text + "'", con)
        sd = New SqlDataAdapter(cmd)
        dt = New DataTable()
        sd.Fill(dt)
        AccountManagementDataGrid.DataSource = dt
